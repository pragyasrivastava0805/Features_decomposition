...
def cal_loss(unrolled_model,unrolled_feature_extractor,unrolled_head_g,input_valid,target_valid):
  domain_features_target,_=unrolled_model(input_valid)
  generalized_features_target = unrolled_feature_extractor(input_valid)
  logits=unrolled_head_g(generalized_features_target-domain_features_target)
  crit = nn.CrossEntropyLoss()
  loss = crit(logits, target_valid)
  return loss
...
class Architect(object):
  ...
  def _compute_unrolled_model(self, input_source,input_target, domain_label_source,domain_label_target, eta, network_optimizer):
    ...
    dtheta = _concat(torch.autograd.grad(loss, self.model.parameters(),retain_graph=True,allow_unused=True)) + self.network_weight_decay*theta
  ...
  def _compute_unrolled_model1(self,input_train_source,input_train_target,class_label_source_train,class_label_target_train,unrolled_model,eta,feature_extractor_optimizer,head_g_optimizer,init_channels):
    domain_features_source,domain_logits_s=self.model(input_train_source)
    domain_features_target,domain_logits_t=self.model(input_train_target)
    ...
    loss=loss1+loss2
    theta1= _concat(self.feature_extractor.parameters())
    theta2= _concat(self.head_g.parameters())
    ...
    dtheta1 = _concat(torch.autograd.grad(loss, self.feature_extractor.parameters(),retain_graph=True,allow_unused=True)) + self.network_weight_decay*theta1
    dtheta2 = _concat(torch.autograd.grad(loss, self.head_g.parameters(),retain_graph=True,allow_unused=True)) + self.network_weight_decay*theta2
  ...
  def step(self, input_train_source,class_label_source_train,domain_label_source_train,input_train_target,class_label_target_train,domain_label_target_train,input_valid, target_valid, eta, network_optimizer,feature_extractor_optimizer,head_g_optimizer,init_channels):
    self.optimizer.zero_grad()
    
    self._backward_step_unrolled(input_train_source, class_label_source_train,domain_label_source_train, input_train_target,class_label_target_train,domain_label_target_train,input_valid,target_valid, eta, network_optimizer,feature_extractor_optimizer,head_g_optimizer,init_channels)
   
    self.optimizer.step()
    
  def step1(self,input_train_source, class_label_source_train,domain_label_source_train, input_train_target,class_label_target_train,domain_label_target_train,input_valid,target_valid,eta, network_optimizer, feature_extractor_optimizer,head_g_optimizer,init_channels):
    self.optimizer.zero_grad()
    ...
    loss=cal_loss(self.model,unrolled_feature_extractor,unrolled_head_g,input_valid,target_valid)
  
    loss.backward(retain_graph=True)
    
    vector_v_dash = [v.grad for v in unrolled_feature_extractor.parameters()]
    
    implicit_grads = self._outer1(vector_v_dash, input_train_source,class_label_source_train,input_train_target,class_label_target_train,domain_label_source_train,domain_label_target_train,self.model,self.feature_extractor,self.head_g, eta)
  ...
  def step2(self,input_train_source, class_label_source_train,domain_label_source_train, input_train_target,class_label_target_train,domain_label_target_train,input_valid,target_valid, eta, network_optimizer, feature_extractor_optimizer,head_g_optimizer,init_channels):
    self.optimizer.zero_grad()
    unrolled_model = self._compute_unrolled_model(input_train_source,input_train_target, domain_label_source_train,domain_label_target_train, eta, network_optimizer)
    unrolled_feature_extractor,unrolled_head_g=self._compute_unrolled_model1(input_train_source,input_train_target,class_label_source_train,class_label_target_train,self.model,eta,feature_extractor_optimizer,head_g_optimizer,init_channels)
    loss=cal_loss(self.model,unrolled_feature_extractor,unrolled_head_g,input_valid,target_valid)
  
    loss.backward(retain_graph=True)
    
    vector_g_dash = [v.grad for v in unrolled_head_g.parameters()]
    
    implicit_grads = self._outer2(vector_g_dash, input_train_source,class_label_source_train,input_train_target,class_label_target_train,domain_label_source_train,domain_label_target_train,self.model,self.feature_extractor,self.head_g, eta)
  ...
  def _backward_step_unrolled(self,input_train_source, class_label_source_train,domain_label_source_train, input_train_target,class_label_target_train,domain_label_target_train,input_valid,target_valid, eta, network_optimizer,feature_extractor_optimizer,head_g_optimizer,init_channels):
    unrolled_model = self._compute_unrolled_model(input_train_source,input_train_target, domain_label_source_train,domain_label_target_train, eta, network_optimizer)
    unrolled_feature_extractor,unrolled_head_g=self._compute_unrolled_model1(input_train_source,input_train_target,class_label_source_train,class_label_target_train,self.model,eta,feature_extractor_optimizer,head_g_optimizer,init_channels)   
    unrolled_loss=cal_loss(self.model,unrolled_feature_extractor,unrolled_head_g,input_valid,target_valid)
  ...
  def _construct_model_from_theta1(self, theta,init_channels):
    model_new = self.feature_extractor.new(init_channels*16)
  ...
  def _outer1(self,vector_v_dash, input_train_source,class_label_source_train,input_train_target,class_label_target_train,domain_label_source_train,domain_label_target_train,unrolled_model,unrolled_feature_extractor,unrolled_head_g, eta, r=1e-2):
    ...
    loss_dash_1.backward(retain_graph=True)
    vector_w_dash=[v.grad.data for v in unrolled_model.parameters()]
    ...
    loss_dash_2.backward(retain_graph=True)
    vector_w_dash=[v.grad.data for v in unrolled_model.parameters()]
